...
export class HomePage {
  ...
  getData(){
    ...
    .then(
      (db:SQLiteObject)=>{
        ...
        db.executeSql('SELECT SUM(amount) AS totalExpence FROM expense WHERE type="Expense"')
          .then(res =>{
            if(res.rows.length>0){
              this.totalExpense = parseInt(res.rows.item(0).totalIncome);
              this.balance = this.totalIncome-this.totalExpense;
            }
          })
          .catch(e => console.log(e));
  ...
  editData(rowid){
      this.navCtrl.push(AddeditdataPage, {rowid:rowid });
  }
  deleteData(rowid){
    this.sqlite.create(
      {
